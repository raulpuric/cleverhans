...
def main(argv=None):
    ...
    for i,layer in enumerate(layers):
        with h5.File(open('cifar_train_adv_act_'+layer+'.h5','r')) as f:
            train_adv=f['data']
        with h5.File(open('cifar_train_act_'+layer+'.h5','r')) as f:
            train=f['data']
        ...
        generate_roc(y,predicts, 'cifar_simple_cross_entropy_'+FLAGS.model+'_train_'+str(layer)+'.png')
        with h5.File(open('cifar_test_adv_act_'+layer+'.h5','r')) as f:
            test_adv=f['data']
        with h5.File(open('cifar_test_act_'+layer+'.h5','r')) as f:
            test=f['data']
        ...
        y=np.concatenate([y_1,y_2],axis=0)
        predicts=clf.predict(x)
        generate_roc(y,predicts, 'cifar_simple_cross_entropy_'+FLAGS.model+'_test_'+str(layer)+'.png')
        print 'test_accuracy: '+str(clf.evaluate(x,y,batch_size=FLAGS.batch))+' for '+layer
        clf.save_weights('cifar_crossentropy_'+layer+'.h5')
